...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    %0:2 = call @inputs() : () -> (tensor<10xui8>, tensor<i32>)
    %1 = call @expected() : () -> tensor<ui8>
    %2 = call @_take(%0#0, %0#1) : (tensor<10xui8>, tensor<i32>) -> tensor<ui8>
    %3 = stablehlo.custom_call @check.eq(%2, %1) : (tensor<ui8>, tensor<ui8>) -> tensor<i1>
    return %3 : tensor<i1>
  }
  func.func private @inputs() -> (tensor<10xui8>, tensor<i32>) {
    %0 = stablehlo.constant dense<[1, 0, 2, 0, 3, 6, 2, 2, 5, 0]> : tensor<10xui8>
    %1 = stablehlo.constant dense<2> : tensor<i32>
    return %0, %1 : tensor<10xui8>, tensor<i32>
  }
  func.func private @expected() -> tensor<ui8> {
    %0 = stablehlo.constant dense<2> : tensor<ui8>
    return %0 : tensor<ui8>
  }
  func.func private @_take(%arg0: tensor<10xui8>, %arg1: tensor<i32>) -> tensor<ui8> {
