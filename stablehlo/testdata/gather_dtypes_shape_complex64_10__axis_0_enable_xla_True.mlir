...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    %0:2 = call @inputs() : () -> (tensor<10xcomplex<f32>>, tensor<i32>)
    %1 = call @expected() : () -> tensor<complex<f32>>
    %2 = call @_take(%0#0, %0#1) : (tensor<10xcomplex<f32>>, tensor<i32>) -> tensor<complex<f32>>
    %3 = stablehlo.custom_call @check.eq(%2, %1) : (tensor<complex<f32>>, tensor<complex<f32>>) -> tensor<i1>
    return %3 : tensor<i1>
  }
  func.func private @inputs() -> (tensor<10xcomplex<f32>>, tensor<i32>) {
    %0 = stablehlo.constant dense<[(2.02702832,-0.957424461), (-1.47250021,-8.45272159), (-0.076321952,3.49267745), (-1.74215984,5.33281422), (0.141330376,5.161990e-01), (-1.09464025,2.38067842), (1.96222961,5.54284096), (0.0646772906,3.86343408), (1.35719633,-5.3441062), (1.37119365,-3.44728732)]> : tensor<10xcomplex<f32>>
    %1 = stablehlo.constant dense<2> : tensor<i32>
    return %0, %1 : tensor<10xcomplex<f32>>, tensor<i32>
  }
  func.func private @expected() -> tensor<complex<f32>> {
    %0 = stablehlo.constant dense<(-0.076321952,3.49267745)> : tensor<complex<f32>>
    return %0 : tensor<complex<f32>>
  }
  func.func private @_take(%arg0: tensor<10xcomplex<f32>>, %arg1: tensor<i32>) -> tensor<complex<f32>> {
