...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    %0:2 = call @inputs() : () -> (tensor<10xf32>, tensor<i32>)
    %1 = call @expected() : () -> tensor<f32>
    %2 = call @_take(%0#0, %0#1) : (tensor<10xf32>, tensor<i32>) -> tensor<f32>
    %3 = stablehlo.custom_call @check.eq(%2, %1) : (tensor<f32>, tensor<f32>) -> tensor<i1>
    return %3 : tensor<i1>
  }
  func.func private @inputs() -> (tensor<10xf32>, tensor<i32>) {
    %0 = stablehlo.constant dense<[-7.79808283, 0.0867945924, 2.22915077, 1.32432771, 1.29375708, 5.35977507, 3.21467471, 4.57879162, -1.26443648, 0.0128132124]> : tensor<10xf32>
    %1 = stablehlo.constant dense<2> : tensor<i32>
    return %0, %1 : tensor<10xf32>, tensor<i32>
  }
  func.func private @expected() -> tensor<f32> {
    %0 = stablehlo.constant dense<2.22915077> : tensor<f32>
    return %0 : tensor<f32>
  }
  func.func private @_take(%arg0: tensor<10xf32>, %arg1: tensor<i32>) -> tensor<f32> {
