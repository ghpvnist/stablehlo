...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    %0:2 = call @inputs() : () -> (tensor<10xi1>, tensor<i32>)
    %1 = call @expected() : () -> tensor<i1>
    %2 = call @_take(%0#0, %0#1) : (tensor<10xi1>, tensor<i32>) -> tensor<i1>
    %3 = stablehlo.custom_call @check.eq(%2, %1) : (tensor<i1>, tensor<i1>) -> tensor<i1>
    return %3 : tensor<i1>
  }
  func.func private @inputs() -> (tensor<10xi1>, tensor<i32>) {
    %0 = stablehlo.constant dense<true> : tensor<10xi1>
    %1 = stablehlo.constant dense<2> : tensor<i32>
    return %0, %1 : tensor<10xi1>, tensor<i32>
  }
  func.func private @expected() -> tensor<i1> {
    %0 = stablehlo.constant dense<true> : tensor<i1>
    return %0 : tensor<i1>
  }
  func.func private @_take(%arg0: tensor<10xi1>, %arg1: tensor<i32>) -> tensor<i1> {
